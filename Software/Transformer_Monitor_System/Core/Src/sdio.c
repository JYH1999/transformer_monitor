...
void Show_SDMMC_Info(void){
    ...
    uint64_t CardCap;                       //SD???
    HAL_SD_GetCardCID(&hsd,&SDCard_CID);    //??CID
    HAL_SD_GetCardInfo(&hsd,&SDCard_INFO);  //??SD???
    switch(SDCard_INFO.CardType)            //SD???
    {
        case CARD_SDSC:
    ...
    CardCap = (uint64_t)(SDCard_INFO.LogBlockNbr)*(uint64_t)(SDCard_INFO.LogBlockSize)/1024/1024;   //??SD???
